...
class TestSeedDeterminism:
    ...
    def test_random_forest_determinism(self, sample_bulk_data, deterministic_seed):
        ...
        model1 = RandomForestClassifier(
            n_estimators=10, random_state=deterministic_seed, max_depth=5
        )
        ...
        assert np.array_equal(pred1, pred2), (
            "RandomForest predictions should be identical with same seed"
        )
        ...
        assert np.allclose(prob1, prob2), (
            "RandomForest probabilities should be identical with same seed"
        )

        # Feature importances should be identical
        assert np.allclose(model1.feature_importances_, model2.feature_importances_)

    def test_logistic_regression_determinism(
        self, sample_bulk_data, deterministic_seed
    ):
        ...
        model1 = LogisticRegression(
            random_state=deterministic_seed, max_iter=1000, solver="lbfgs"
        )
    ...
    def test_neural_network_determinism(self, sample_bulk_data, deterministic_seed):
        ...
        assert agreement > 0.95, (
            f"Neural network predictions should be highly consistent, got {agreement:.3f} agreement"
        )
    ...
    def test_train_test_split_determinism(self, sample_bulk_data, deterministic_seed):
        ...
        assert X_train1.index.equals(X_train2.index), (
            "Training sets should be identical"
        )
    ...
    def test_umap_determinism(self, sample_single_cell_data, deterministic_seed):
        ...
        flipped_x = np.allclose(umap1[:, 0], -umap2[:, 0], atol=1e-5) and np.allclose(
            umap1[:, 1], umap2[:, 1], atol=1e-5
        )
        flipped_y = np.allclose(umap1[:, 0], umap2[:, 0], atol=1e-5) and np.allclose(
            umap1[:, 1], -umap2[:, 1], atol=1e-5
        )
        flipped_both = np.allclose(
            umap1[:, 0], -umap2[:, 0], atol=1e-5
        ) and np.allclose(umap1[:, 1], -umap2[:, 1], atol=1e-5)

        is_reproducible = direct_match or flipped_x or flipped_y or flipped_both

        assert is_reproducible, (
            "UMAP embeddings should be reproducible (allowing for axis flips)"
        )

    def test_leiden_clustering_determinism(
        self, sample_single_cell_data, deterministic_seed
    ):
        ...
        assert np.array_equal(clusters1, clusters2), (
            "Leiden clustering should be deterministic with fixed seed"
        )
    ...
    def test_pca_determinism(self, sample_bulk_data, deterministic_seed):
        ...
        pca1 = PCA(
            n_components=10, svd_solver="randomized", random_state=deterministic_seed
        )
        pca2 = PCA(
            n_components=10, svd_solver="randomized", random_state=deterministic_seed
        )
        ...
        for i in range(10):
            component_match = np.allclose(
                result1[:, i], result2[:, i], atol=1e-10
            ) or np.allclose(result1[:, i], -result2[:, i], atol=1e-10)
    ...
    def test_cross_validation_determinism(self, sample_bulk_data, deterministic_seed):
        ...
        for i, ((train1, test1), (train2, test2)) in enumerate(zip(folds1, folds2)):
            assert np.array_equal(train1, train2), (
                f"KFold training indices should be identical for fold {i}"
            )
            assert np.array_equal(test1, test2), (
                f"KFold test indices should be identical for fold {i}"
            )

        # Test StratifiedKFold
        skfold1 = StratifiedKFold(
            n_splits=3, shuffle=True, random_state=deterministic_seed
        )
        ...
        for i, ((strain1, stest1), (strain2, stest2)) in enumerate(
            zip(sfolds1, sfolds2)
        ):
            assert np.array_equal(strain1, strain2), (
                f"StratifiedKFold training indices should be identical for fold {i}"
            )
            assert np.array_equal(stest1, stest2), (
                f"StratifiedKFold test indices should be identical for fold {i}"
            )
...
class TestNonDeterministicBehavior:
    ...
    def test_unseeded_randomness_detection(self, sample_bulk_data):
        ...
        importance_diff = np.mean(
            np.abs(model1.feature_importances_ - model2.feature_importances_)
        )
        ...
        assert isinstance(importance_diff, float), (
            "Should compute importance difference"
        )
    ...
    def test_seed_isolation(self, sample_bulk_data, deterministic_seed):
        ...
        model1 = RandomForestClassifier(
            n_estimators=10, random_state=deterministic_seed
        )
        model2 = RandomForestClassifier(
            n_estimators=10, random_state=deterministic_seed + 1
        )
        ...
        assert importance_diff > 1e-10, (
            "Different seeds should produce different feature importances"
        )
        ...
        assert prediction_agreement < 1.0, (
            "Different seeds should produce some different predictions"
        )
...
class TestEnvironmentReproducibility:
    ...
    def test_numpy_seed_behavior(self, deterministic_seed):
        ...
        # Should be identical
        assert np.array_equal(numbers1, numbers2), (
            "Numpy random numbers should be identical with same seed"
        )
        ...
        assert not np.array_equal(numbers1, numbers3), (
            "Different numpy seeds should give different results"
        )

    def test_sklearn_random_state_propagation(
        self, sample_bulk_data, deterministic_seed
    ):
        ...
        assert np.array_equal(pred1, pred2), (
            "Pipeline predictions should be identical with same random_state"
        )
    ...
    @pytest.mark.skip(
        reason="Scanpy highly_variable_genes has issues with small datasets"
    )
    def test_scanpy_random_state_settings(
        self, sample_single_cell_data, deterministic_seed
    ):
        ...
        assert np.array_equal(hvg1, hvg2), (
            "Highly variable gene selection should be deterministic"
        )
