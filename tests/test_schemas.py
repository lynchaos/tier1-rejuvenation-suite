...
class TestDataSchemas:
    ...
    def test_bulk_data_schema_validation(self, sample_bulk_data, temp_dir):
        ...
        assert data.index.equals(metadata.index), "Data and metadata indices should align"
        ...
        assert metadata['condition'].dtype == 'object' or metadata['condition'].dtype.name == 'category', \
        ...
        metadata_loaded = pd.read_csv(metadata_path, index_col=0)
    ...
    def test_single_cell_schema_validation(self, sample_single_cell_data, temp_dir):
        ...
        assert pd.api.types.is_numeric_dtype(adata.obs['n_genes']), "n_genes should be numeric"
        assert pd.api.types.is_numeric_dtype(adata.obs['n_counts']), "n_counts should be numeric"
        ...
        computed_n_genes = (adata.X > 0).sum(axis=1).A1 if hasattr(adata.X, 'A1') else (adata.X > 0).sum(axis=1)
        assert np.allclose(computed_n_genes, adata.obs['n_genes'].values), \
            "Stored n_genes should match computed values"
        
        computed_n_counts = adata.X.sum(axis=1).A1 if hasattr(adata.X, 'A1') else adata.X.sum(axis=1)
        assert np.allclose(computed_n_counts, adata.obs['n_counts'].values, rtol=1e-3), \
        ...
        assert adata_loaded.obs.columns.equals(adata.obs.columns), "Obs columns should be preserved"
        assert adata_loaded.var.columns.equals(adata.var.columns), "Var columns should be preserved"
        ...
        assert np.allclose(original_matrix, loaded_matrix), "Expression matrix should be preserved"
    ...
    def test_multi_omics_schema_validation(self, sample_multi_omics_data, temp_dir):
        ...
        for omics_type, data in omics_data.items():
            if omics_type != 'metadata':
                assert isinstance(data, pd.DataFrame), f"{omics_type} should be DataFrame"
        ...
        for omics_type, data in omics_data.items():
            if omics_type != 'metadata':
                assert data.dtypes.apply(lambda x: pd.api.types.is_numeric_dtype(x)).all(), \
                    f"All {omics_type} features should be numeric"
        
        # Test feature name uniqueness within each omics type
        for omics_type, data in omics_data.items():
            if omics_type != 'metadata':
                assert data.columns.is_unique, f"{omics_type} feature names should be unique"
        ...
        assert prot_data.notna().any().any(), "Proteomics should have some non-missing values"
        ...
        for omics_type, data in omics_data.items():
            ...
            assert loaded_data.shape == data.shape, f"{omics_type} shape preservation failed"
    ...
    def test_missing_value_patterns(self, sample_bulk_data):
        ...
        assert max_missing_per_sample < data.shape[1], "No sample should be completely missing"
        
        # No feature should be completely missing
        assert max_missing_per_feature < data.shape[0], "No feature should be completely missing"
        ...
        high_missing_samples = missing_per_sample[missing_per_sample > 0.8 * data.shape[1]]
        ...
        high_missing_features = missing_per_feature[missing_per_feature > 0.5 * data.shape[0]]
        if len(high_missing_features) > 0:
            warnings.warn(f"Found {len(high_missing_features)} features with >50% missing values")
    ...
    def test_data_type_consistency(self, sample_bulk_data, temp_dir):
        ...
        formats_to_test = [
            ('csv', lambda df, path: df.to_csv(path), lambda path: pd.read_csv(path, index_col=0)),
            ('parquet', lambda df, path: df.to_parquet(path), lambda path: pd.read_parquet(path)),
            ('pickle', lambda df, path: df.to_pickle(path), lambda path: pd.read_pickle(path))
        ]
        ...
        for format_name, save_func, load_func in formats_to_test:
            try:
                ...
                assert loaded_data.shape == data.shape, f"{format_name}: Shape not preserved"
                assert loaded_data.index.equals(data.index), f"{format_name}: Index not preserved"
                assert loaded_data.columns.equals(data.columns), f"{format_name}: Columns not preserved"
                
                # Check data values (handle NaN properly)
                data_equal = np.allclose(loaded_data.values, data.values, equal_nan=True)
                ...
                assert loaded_metadata.shape == metadata.shape, f"{format_name}: Metadata shape not preserved"
                
            except Exception as e:
                # Some formats might not be available
                warnings.warn(f"Could not test {format_name} format: {e}")
    ...
    def test_column_name_validation(self, sample_bulk_data):
        ...
        for col in all_columns:
            try:
                ...
                warnings.warn(f"Non-ASCII column name detected: '{col}'")
    ...
    def test_numeric_data_validation(self, sample_bulk_data):
        ...
        if len(finite_data) > 0:
            ...
            assert max_abs_value < 1e10, "Extremely large values detected (possible data error)"
        ...
        if 'gene_' in str(data.columns[0]):  # Assume gene expression data
            ...
            if negative_count > 0:
                warnings.warn(f"Found {negative_count} negative values in expression data")
        ...
        for col in numeric_cols[:10]:  # Test subset for performance
            ...
            if len(col_data) > 0:
                ...
                if data_std > 0:
                    ...
                    assert range_to_std_ratio > 0.1, f"Suspiciously low dynamic range in {col}"
...
class TestFileFormatValidation:
    ...
    def test_h5ad_file_integrity(self, sample_single_cell_data, temp_dir):
        ...
        adata.obs['test_categorical'] = pd.Categorical(['A', 'B', 'C'] * (len(adata) // 3 + 1))[:len(adata)]
        ...
        assert isinstance(adata_loaded.obs['test_categorical'].dtype, pd.CategoricalDtype)
        ...
        assert file_size < 100_000_000, "File should not be excessively large for test data"
    ...
    def test_parquet_schema_preservation(self, sample_bulk_data, temp_dir):
        ...
        test_metadata['category_col'] = pd.Categorical(np.random.choice(['A', 'B', 'C'], len(metadata)))
        ...
        try:
            ...
            assert isinstance(loaded_metadata['category_col'].dtype, pd.CategoricalDtype)
            
            # Check index preservation
            assert loaded_metadata.index.equals(test_metadata.index)
            
        except ImportError:
            warnings.warn("Parquet support not available (pyarrow not installed)")
    ...
    def test_pickle_security_and_integrity(self, sample_bulk_data, temp_dir):
        ...
        complex_metadata['list_col'] = [list(range(i, i+3)) for i in range(len(metadata))]
        complex_metadata['dict_col'] = [{'key': i, 'value': i**2} for i in range(len(metadata))]
...
class TestDataValidationUtils:
        ...
        if numeric_columns:
            for col in numeric_columns:
                if col in df.columns:
                    results[f'{col}_is_numeric'] = pd.api.types.is_numeric_dtype(df[col])
    ...
    def validate_anndata_schema(adata: ad.AnnData) -> Dict[str, bool]:
        ...
        if adata.X is not None:
            ...
            results['finite_values'] = np.all(np.isfinite(adata.X.data)) if hasattr(adata.X, 'data') else np.all(np.isfinite(adata.X))
