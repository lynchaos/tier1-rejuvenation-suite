...
class TestNormalization:
    ...
    def test_log_normalization(self, sample_bulk_data):
        ...
        if len(original_finite) > 0 and len(log_finite) > 0:
            ...
            original_values = original_finite.values.flatten()
            log_values = log_finite.values.flatten()
            
            # Basic sanity checks
            assert len(log_values) > 0, "Should have some transformed values"
            assert np.all(np.isfinite(log_values)), "All non-NaN values should be finite"
...
class TestPCA:
    ...
    def test_pca_basic_properties(self, sample_bulk_data):
        ...
        scaled_df = pd.DataFrame(scaled_data, index=clean_data.index, columns=clean_data.columns)
    ...
    def test_pca_reproducibility(self, sample_bulk_data, deterministic_seed):
        ...
        pca_result1, pca_model1 = apply_pca(scaled_df, n_components=5, random_state=deterministic_seed)
        
        np.random.seed(deterministic_seed)
        pca_result2, pca_model2 = apply_pca(scaled_df, n_components=5, random_state=deterministic_seed)
        
        # Results should be identical (up to sign flip)
        for i in range(5):
            # Components might be flipped in sign
            assert np.allclose(pca_result1.iloc[:, i], pca_result2.iloc[:, i], atol=1e-10) or \
                   np.allclose(pca_result1.iloc[:, i], -pca_result2.iloc[:, i], atol=1e-10)
    ...
    def test_pca_variance_explained(self, sample_bulk_data):
        ...
        structured_data = np.outer(component1, loadings1) + np.outer(component2, loadings2) + noise
        ...
        assert pca_model.explained_variance_ratio_[0] > pca_model.explained_variance_ratio_[1]
        assert pca_model.explained_variance_ratio_[0] > 0.1  # Should capture substantial variance
...
class TestScaling:
    ...
    def test_robust_scaling(self, sample_bulk_data):
        ...
        feature_iqrs = scaled_data.quantile(0.75, axis=0) - scaled_data.quantile(0.25, axis=0)
