...
def validate_test_environment() -> Dict[str, bool]:
    ...
    validation_results['test_directory_readable'] = test_dir.is_dir() and os.access(test_dir, os.R_OK)
    
    # Check for test files
    expected_test_files = [
        'test_transforms.py', 'test_leakage.py', 
        'test_determinism.py', 'test_schemas.py'
    ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TIER 1 Rejuvenation Suite Test Runner")
    parser.add_argument(
        '--categories', 
        nargs='*', 
    ...
    parser.add_argument('--benchmark', action='store_true', help='Benchmark test performance')
    ...
    if args.validate:
        ...
        critical_checks = [
            'pytest_available', 'numpy_available', 'pandas_available', 
            'scikit-learn_available', 'test_directory_exists'
        ]
        
        all_critical_passed = all(validation.get(check, False) for check in critical_checks)
