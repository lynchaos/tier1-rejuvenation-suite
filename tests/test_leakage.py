...
class TestDataLeakage:
    ...
    def test_scaler_leakage_detection(self, sample_bulk_data):
        ...
        train_difference = np.mean(
            np.abs(X_train_scaled_correct - X_train_scaled_incorrect)
        )
        test_difference = np.mean(
            np.abs(X_test_scaled_correct - X_test_scaled_incorrect)
        )
        ...
        assert not np.allclose(
            scaler_correct.scale_, scaler_incorrect.scale_, rtol=1e-3
        )
    ...
    def test_feature_selection_leakage(self, sample_bulk_data):
        ...
        selector_correct.fit_transform(X_train, y_train)
        selector_correct.transform(X_test)
        ...
        selector_incorrect.fit_transform(X_clean, y_clean)
    ...
    def test_imputation_leakage(self):
        ...
        train_diff = np.nanmean(
            np.abs(X_train_imputed_correct.values - X_train_imputed_incorrect.values)
        )
        test_diff = np.nanmean(
            np.abs(X_test_imputed_correct.values - X_test_imputed_incorrect.values)
        )
    ...
    def test_filter_leakage_variance(self, sample_bulk_data):
        ...
        X_train[high_var_features]
        X_test[high_var_features]

        # INCORRECT: Variance filtering based on all data
        all_variances = X.var()
        all_variance_threshold = all_variances.quantile(0.1)
        all_high_var_features = all_variances[
            all_variances >= all_variance_threshold
        ].index

        # Feature sets should potentially be different
        len(set(high_var_features) & set(all_high_var_features))
        len(set(high_var_features) | set(all_high_var_features))
    ...
    def test_end_to_end_pipeline_leakage(self, sample_bulk_data):
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X):
            ...
            for inner_train_idx, inner_val_idx in inner_cv.split(X_outer_train):
                ...
                scaler = StandardScaler()
                X_inner_train_processed = scaler.fit_transform(
            ...
            outer_threshold = outer_variances.quantile(
                best_config["variance_threshold"]
            )
            outer_selected_features = outer_variances[
                outer_variances >= outer_threshold
            ].index

            outer_scaler = StandardScaler()
            X_outer_train_final = outer_scaler.fit_transform(
                X_outer_train[outer_selected_features]
            )
            X_outer_test_final = outer_scaler.transform(
                X_outer_test[outer_selected_features]
            )
...
class TestPreprocessingLeakagePatterns:
    ...
    def test_global_statistics_leakage(self, sample_bulk_data):
        ...
        for stat_name in train_stats:
            assert not np.allclose(
                train_stats[stat_name].values, all_stats[stat_name].values, rtol=1e-3
    ...
    def test_temporal_leakage_simulation(self):
        ...
        X_correct = np.column_stack(
            [
                ...
                ),  # This includes current value - problematic!
            ]
        )
        ...
        model_incorrect.score(X_test_incorrect, y_test_binary)
