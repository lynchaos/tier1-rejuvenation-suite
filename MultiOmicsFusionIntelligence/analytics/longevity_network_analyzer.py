...
class LongevityNetworkAnalyzer:
    ...
    def load_gene_network(self, network_path: str = None) -> nx.Graph:
        ...
        longevity_genes = [
            'SIRT1', 'SIRT3', 'SIRT6', 'FOXO1', 'FOXO3', 'TP53', 'mTOR', 'IGF1R',
            'APOE', 'KLOTHO', 'TERT', 'TERF1', 'CDKN2A', 'CDKN1A', 'NRF2', 'PGC1A',
            'AMPK', 'PTEN', 'RB1', 'p16', 'p21', 'ATM', 'WRN', 'BLM', 'LMNA'
        ]
        
        # Add aging-related genes
        aging_genes = [
            'TNF', 'IL6', 'IL1B', 'NFKB1', 'JUN', 'FOS', 'MYC', 'RAS', 'AKT1',
            'PIK3CA', 'VEGFA', 'HIF1A', 'EGFR', 'PDGFRA', 'TGFB1', 'SMAD3'
        ]
        ...
        mapping = {i: gene for i, gene in enumerate(all_genes)}
        ...
        # Add node attributes
        for node in G.nodes():
            G.nodes[node]['gene_type'] = 'longevity' if node in longevity_genes else \
                                       'aging' if node in aging_genes else 'other'
    ...
    def identify_network_modules(self, n_clusters: int = 10) -> Dict[str, List[str]]:
        ...
        for i in range(n_clusters):
            cluster_genes = [gene_names[j] for j, label in enumerate(cluster_labels) if label == i]
            modules[f'Module_{i}'] = cluster_genes
        
        print(f"Identified {len(modules)} network modules")
        return modules
    
    def pathway_impact_scoring(self, gene_expression: pd.DataFrame, 
        ...
        for pathway_name, pathway_genes in pathways.items():
            ...
            if len(available_genes) > 0:
                ...
                if self.gene_network:
                    network_genes = [g for g in available_genes if g in self.gene_network.nodes()]
                    if network_genes:
    ...
    def prioritize_intervention_targets(self, centrality_df: pd.DataFrame, 
                                      pathway_df: pd.DataFrame, 
                                      druggability_data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """
        ...
        target_scores['pathway_score'] = [pathway_membership.get(gene, 0) 
        ...
        # Sort by priority
        target_scores = target_scores.sort_values('intervention_priority', ascending=False)
    ...
    def visualize_network(self, centrality_df: pd.DataFrame, top_n: int = 20) -> None:
        ...
        color_map = {'longevity': 'lightgreen', 'aging': 'lightcoral', 'other': 'lightblue'}
        node_colors = [color_map[subgraph.nodes[node]['gene_type']] for node in subgraph.nodes()]
        
        # Node sizes based on centrality scores
        node_sizes = [centrality_df.loc[node, 'composite_score'] * 1000 for node in subgraph.nodes()]
        ...
        legend_elements = [plt.Line2D([0], [0], marker='o', color='w', 
                                    markerfacecolor=color, markersize=10, label=gene_type)
    ...
    def run_longevity_analysis(self, expression_data: Optional[pd.DataFrame] = None) -> Dict:
        """
        ...
        if expression_data is not None:
            ...
            # Prioritize targets
            target_priorities = self.prioritize_intervention_targets(centrality_df, pathway_df)
        else:
            pathway_df = pd.DataFrame()
...
if __name__ == '__main__':
    ...
    print(results['intervention_targets'].head(10)[['gene_type', 'intervention_priority']])
