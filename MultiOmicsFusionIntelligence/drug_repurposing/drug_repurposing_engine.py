...
class DrugRepurposingEngine:
    ...
    def load_drug_databases(self) -> Dict[str, pd.DataFrame]:
        ...
        drugs = [
            'Metformin', 'Rapamycin', 'Resveratrol', 'Nicotinamide', 'Spermidine',
            'Lithium', 'Aspirin', 'Statins', 'ACE_inhibitors', 'Metoprolol',
            'Dasatinib', 'Quercetin', 'Fisetin', 'Curcumin', 'NAD_precursors',
            'Senolytics', 'mTOR_inhibitors', 'AMPK_activators', 'Sirtuin_activators',
            'Antioxidants', 'Anti_inflammatories', 'Autophagy_inducers'
        ]
        ...
        drug_properties = pd.DataFrame({
            ...
            'clinical_trial_phase': np.random.choice(['Preclinical', 'Phase I', 'Phase II', 'Phase III', 'FDA Approved'], len(drugs))
        })
        
        # Drug-target interactions
        targets = [
            'mTOR', 'AMPK', 'SIRT1', 'SIRT3', 'FOXO1', 'FOXO3', 'TP53', 'NFE2L2',
            'NFKB1', 'IGF1R', 'PIK3CA', 'AKT1', 'PTEN', 'CDKN1A', 'CDKN2A',
            'TERT', 'KLOTHO', 'TNF', 'IL6', 'IL1B', 'SOD1', 'SOD2', 'CAT'
        ]
        
        # Create drug-target interaction matrix
        drug_target_interactions = np.random.choice([0, 1], size=(len(drugs), len(targets)), p=[0.8, 0.2])
        drug_target_df = pd.DataFrame(drug_target_interactions, index=drugs, columns=targets)
        
        # Drug mechanisms of action
        mechanisms = [
            'Autophagy induction', 'Senescence clearance', 'Oxidative stress reduction',
            'Inflammation suppression', 'Metabolic enhancement', 'DNA repair activation',
            'Telomere maintenance', 'Protein homeostasis', 'Mitochondrial function',
            'Insulin sensitivity', 'Growth factor modulation', 'Cell cycle regulation'
        ]
        
        drug_mechanisms = pd.DataFrame(
            np.random.choice([0, 1], size=(len(drugs), len(mechanisms)), p=[0.7, 0.3]),
            index=drugs, columns=mechanisms
        )
    ...
    def create_expression_signatures(self, expression_data: pd.DataFrame, 
                                   condition_labels: pd.Series) -> Dict[str, pd.Series]:
        """
        ...
        for condition in conditions:
            ...
            if len(condition_samples) > 0 and len(other_samples) > 0:
                ...
                signature['score'] = -np.log10(signature['p_value'] + 1e-10) * np.sign(signature['log2fc'])
    ...
    def calculate_connectivity_scores(self, drug_signatures: Dict[str, pd.Series]) -> pd.DataFrame:
        """
        ...
        if self.expression_signatures is None:
            raise ValueError("Expression signatures not created. Please run create_expression_signatures first.")
        ...
        for drug_name, drug_signature in drug_signatures.items():
            ...
            for condition_name, condition_signature in self.expression_signatures.items():
                # Find common genes
                common_genes = drug_signature.index.intersection(condition_signature.index)
                ...
                if len(common_genes) > 50:  # Minimum overlap
                    ...
                    drug_connectivity.append(-connectivity_score)  # Flip sign for therapeutic potential
                else:
    ...
    def identify_repurposing_candidates(self, target_condition: str = 'aged', 
        ...
        if target_condition not in self.connectivity_scores.columns:
            raise ValueError(f"Condition {target_condition} not found in connectivity scores.")
        
        # Get connectivity scores for target condition
        condition_scores = self.connectivity_scores[target_condition].sort_values(ascending=False)
        ...
        for drug in condition_scores.head(top_n).index:
            if drug in drug_properties.index:
                ...
                development_score = {'Preclinical': 0.2, 'Phase I': 0.4, 'Phase II': 0.6, 
                                   'Phase III': 0.8, 'FDA Approved': 1.0}[drug_info['clinical_trial_phase']]
                
                repurposing_score = connectivity * 0.5 + safety_score * 0.3 + development_score * 0.2
        ...
        repurposing_df = repurposing_df.sort_values('repurposing_score', ascending=False)
        
        print(f"Identified {len(repurposing_df)} repurposing candidates")
        return repurposing_df
    
    def analyze_therapeutic_targets(self, repurposing_candidates: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        target_categories = {
            'mTOR': 'Growth signaling', 'AMPK': 'Energy metabolism', 
            'SIRT1': 'Longevity', 'SIRT3': 'Longevity',
            'FOXO1': 'Stress response', 'FOXO3': 'Stress response',
            'TP53': 'DNA damage', 'NFE2L2': 'Antioxidant response',
            'NFKB1': 'Inflammation', 'IGF1R': 'Growth signaling',
            'TNF': 'Inflammation', 'IL6': 'Inflammation'
        }
        
        target_df['category'] = target_df['target'].map(lambda x: target_categories.get(x, 'Other'))
        
        print(f"Analyzed {len(target_df)} therapeutic targets")
        return target_df
    
    def create_drug_target_network(self, repurposing_candidates: pd.DataFrame, 
                                 target_analysis: pd.DataFrame) -> nx.Graph:
        """
        ...
        for _, candidate in repurposing_candidates.head(15).iterrows():
            ...
            if drug in drug_targets.index:
                for target in target_analysis.head(10)['target']:
                    if target in drug_targets.columns and drug_targets.loc[drug, target] == 1:
    ...
    def visualize_repurposing_results(self, repurposing_candidates: pd.DataFrame,
                                    target_analysis: pd.DataFrame) -> None:
        """
        ...
        fig.suptitle('Drug Repurposing Analysis Results', fontsize=16, fontweight='bold')
        ...
        # 2. Score components
        score_components = top_candidates[['connectivity_score', 'safety_score', 'development_score']].head(8)
        ...
        axes[0, 2].pie(phase_counts.values, labels=phase_counts.index, autopct='%1.1f%%')
        ...
        axes[1, 2].scatter(repurposing_candidates['bioavailability'], 
    ...
    def generate_repurposing_report(self, repurposing_candidates: pd.DataFrame,
                                  target_analysis: pd.DataFrame) -> str:
        """
        ...
        report.append(f"Top repurposing score: {repurposing_candidates['repurposing_score'].max():.3f}")
        report.append(f"Average repurposing score: {repurposing_candidates['repurposing_score'].mean():.3f}")
        ...
        report.append("-" * 40)
        for i, (_, candidate) in enumerate(repurposing_candidates.head(10).iterrows(), 1):
            report.append(f"{i:2d}. {candidate['drug']:<15} "
        ...
        for i, (_, target) in enumerate(target_analysis.head(10).iterrows(), 1):
            report.append(f"{i:2d}. {target['target']:<10} "
    ...
    def run_repurposing_pipeline(self, expression_data: pd.DataFrame,
                               condition_labels: pd.Series,
                               target_condition: str = 'aged') -> Dict:
        """
        ...
        signatures = self.create_expression_signatures(expression_data, condition_labels)
        ...
        for drug in self.drug_database['properties']['drug_name']:
            ...
            signature = pd.Series(np.random.normal(0, 1, len(expression_data.columns)),
        ...
        # Create network
        drug_target_network = self.create_drug_target_network(repurposing_candidates, target_analysis)
        
        # Visualization
        self.visualize_repurposing_results(repurposing_candidates, target_analysis)
        
        # Generate report
        report = self.generate_repurposing_report(repurposing_candidates, target_analysis)
...
if __name__ == '__main__':
    ...
    print(results['repurposing_candidates'].head()[['drug', 'repurposing_score', 'clinical_phase']])
