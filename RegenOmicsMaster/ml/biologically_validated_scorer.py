...
class BiologicallyValidatedRejuvenationScorer:
    ...
    def __init__(self, model_dir: str = "models/", random_state: int = 42):
        ...
        self.aging_markers = {
            "cellular_senescence": [
                ...
                "CDKN2A",
                "CDKN2B",
                "CDKN1B",  # Cell cycle inhibitors (Campisi & d'Adda di Fagagna, 2007)
                "TP53",
                "TP21",
                "RB",  # DNA damage response (Rodier & Campisi, 2011)
                "GLB1",
                "LMNB1",  # Senescence-associated markers (Hernandez-Segura et al., 2018)
            ],
            "sasp_inflammation": [  # Senescence-Associated Secretory Phenotype
                ...
                "IL6",
                "IL8",
                "TNF",  # Core SASP factors (Coppé et al., 2008)
                "CXCL1",
                "CXCL2",
                "CCL2",
                "CCL20",  # Chemokines (Acosta et al., 2013)
                "NFKB1",
                "RELA",
                "JUN",
                "FOS",  # Inflammatory TFs (Salminen et al., 2012)
            ],
            "dna_damage_response": [
                "ATM",
                "ATR",
                ...
                "CHEK2",
                "BRCA1",
                "BRCA2",  # DNA repair (Jackson & Bartek, 2009)
                "H2AFX",
                "MDC1",
                "RAD51",
                "PARP1",  # DNA damage signaling (Kuilman et al., 2010)
            ],
            "telomere_dysfunction": [
                ...
                "TERF2",
                "TERF2IP",
                "TINF2",  # Telomere biology (Blackburn et al., 2015)
                "POT1",
                "CTC1",
                "RTEL1",
                "WRAP53",  # Telomere maintenance (Armanios & Blackburn, 2012)
            ],
            "oxidative_stress": [
                "SOD1",
                "SOD2",
                "CAT",
                "GPX1",
                "GPX4",  # Antioxidant enzymes (Finkel & Holbrook, 2000)
                "NQO1",
                "GCLC",
                "GSR",
                "PRDX1",
                "PRDX3",  # Oxidative stress response (Gems & Doonan, 2009)
            ],
            "mitochondrial_dysfunction": [  # Added missing hallmark
                "TFAM",
                "NRF1",
                "NRF2",
                "PGC1A",  # Mitochondrial biogenesis (Scarpulla, 2008)
                "COX4I1",
                "COX5A",
                "CYTB",
                "ND1",  # Mitochondrial function (López-Otín et al., 2013)
            ],
        }
        ...
        self.rejuvenation_markers = {
            "longevity_pathways": [
                ...
                "SIRT3",
                "SIRT6",
                "SIRT7",  # Sirtuins (Haigis & Sinclair, 2010)
                "FOXO1",
                "FOXO3",
                "FOXO4",  # FOXO TFs (Martins et al., 2016)
                "KLOTHO",
                "FGF21",
                "GDF11",  # Longevity hormones (Kurosu et al., 2005)
            ],
            "metabolic_rejuvenation": [
                "PRKAA1",
                "PRKAA2",  # AMPK subunits (Hardie, 2007)
                "PPARGC1A",
                "PPARA",
                "PPARG",  # PGC-1α and PPARs (Finck & Kelly, 2006)
                "NRF1",
                "NRF2",
                "TFAM",
                "MTOR",  # Metabolic regulators (Scarpulla, 2008)
            ],
            "autophagy_quality_control": [
                "ATG5",
                "ATG7",
                "ATG12",
                "BECN1",  # Autophagy core (Levine & Kroemer, 2008)
                "MAP1LC3A",
                "MAP1LC3B",
                "SQSTM1",  # LC3 and p62 (Klionsky et al., 2016)
                "PINK1",
                "PRKN",
                "ULK1",  # Mitophagy (Narendra et al., 2008)
            ],
            "stem_cell_pluripotency": [
                "POU5F1",
                "SOX2",
                "NANOG",
                "KLF4",  # Yamanaka factors (Takahashi & Yamanaka, 2006)
                "MYC",
                "LIN28A",
                "UTF1",
                "DPPA4",  # Pluripotency network (Boyer et al., 2005)
            ],
            "epigenetic_rejuvenation": [
                "TET1",
                "TET2",
                "TET3",  # DNA demethylation (Tahiliani et al., 2009)
                "DNMT1",
                "DNMT3A",
                "DNMT3B",  # DNA methylation (Li et al., 1992)
                "KDM4A",
                "KDM6A",
                "JMJD3",  # Histone demethylases (Klose & Zhang, 2007)
            ],
            "tissue_regeneration": [  # Added tissue-specific markers
                "WNT3A",
                "WNT10B",
                "LGR5",
                "BMI1",  # Stem cell maintenance (Clevers, 2013)
                "NOTCH1",
                "DLL1",
                "JAG1",
                "HES1",  # Notch signaling (Artavanis-Tsakonas et al., 1999)
            ],
        }
    ...
    def load_data(
        self, de_path: str, metadata_path: Optional[str] = None
    ) -> pd.DataFrame:
        """
    ...
    def calculate_pathway_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for pathway, markers in self.aging_markers.items():
            ...
            if available_markers:
                ...
                pathway_scores[f"aging_{pathway}_score"] = df[available_markers].median(
                    axis=1
                )
                ...
                logger.info(
                    f"Aging {pathway}: {len(available_markers)}/{len(markers)} markers found"
                )
            else:
        ...
        for pathway, markers in self.rejuvenation_markers.items():
            available_markers = [m for m in markers if m in df.columns]
            if available_markers:
                pathway_scores[f"rejuv_{pathway}_score"] = df[available_markers].median(
                    axis=1
                )
                ...
                logger.info(
                    f"Rejuvenation {pathway}: {len(available_markers)}/{len(markers)} markers found"
                )
            else:
    ...
    def create_biologically_validated_target(self, df: pd.DataFrame) -> pd.Series:
        ...
        if target_components:
            ...
            logger.warning(
                "No biological components available. Creating age-based synthetic target."
            )
            target_normalized = self._create_age_based_target(df)

        return pd.Series(
            target_normalized, index=df.index, name="biological_rejuvenation_score"
        )

    def _create_age_strata(
        self, ages: Optional[np.ndarray], n_samples: int = None
    ) -> np.ndarray:
    ...
    def _age_stratify_component(
        self, component: pd.Series, age_strata: np.ndarray, component_name: str
    ) -> np.ndarray:
        """
    ...
    def _age_stratified_normalization(
        self, target: np.ndarray, age_strata: np.ndarray
    ) -> np.ndarray:
        """
        ...
        for stratum in ["young", "middle", "old"]:
            ...
            if np.any(mask):
                ...
                mad_val = np.median(
                    np.abs(stratum_values - median_val)
                )  # Median Absolute Deviation

                if mad_val > 0:
                    # Robust z-score normalization
                    normalized_stratum = (stratum_values - median_val) / (
                        1.4826 * mad_val
                    )
    ...
    def _create_age_based_target(self, df: pd.DataFrame) -> np.ndarray:
        ...
        if "age" in df.columns:
            ...
            min_age = ages.min()
            age_normalized = (
                (max_age - ages) / (max_age - min_age)
                if max_age != min_age
                else np.full(len(ages), 0.5)
            )
    ...
    def train_ensemble_models(
        self, X: pd.DataFrame, y: pd.Series, age_strata: Optional[np.ndarray] = None
    ) -> Dict:
        """
        ...
        X_scaled_df = pd.DataFrame(
            X_scaled, columns=feature_cols, index=X_features.index
        )
    ...
    def _calculate_biological_importance(
        self, model, X_scaled_df: pd.DataFrame, model_name: str
    ) -> pd.Series:
        """
        ...
        for feature in importance_series.index:
            ...
            for pathway_type in ["aging_markers", "rejuvenation_markers"]:
                ...
                for _pathway_name, markers in pathways.items():
    ...
    def _create_biological_categories(
        self, scores: pd.Series, age_strata: np.ndarray
    ) -> pd.Series:
        """
