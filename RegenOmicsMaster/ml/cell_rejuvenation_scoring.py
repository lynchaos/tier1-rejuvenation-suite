...
class CellRejuvenationScorer:
    ...
    def calculate_pathway_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for pathway, markers in self.aging_markers.items():
            ...
            if available_markers:
                pathway_scores[f"aging_{pathway}_score"] = df[available_markers].mean(
                    axis=1
                )
        ...
        for pathway, markers in self.rejuvenation_markers.items():
            available_markers = [m for m in markers if m in df.columns]
            if available_markers:
                pathway_scores[f"rejuv_{pathway}_score"] = df[available_markers].mean(
                    axis=1
                )
                logger.info(
                    f"Rejuvenation {pathway}: {len(available_markers)} markers found"
                )
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        aging_cols = [
            ...
            if isinstance(col, str) and col.startswith("aging_")
        ]
        if aging_cols:
        ...
        rejuv_cols = [
            col
            for col in df.columns
            if isinstance(col, str) and col.startswith("rejuv_")
        ]
        if rejuv_cols:
        ...
        if (
            "composite_aging_score" in featured_df.columns
            and "composite_rejuv_score" in featured_df.columns
        ):
            featured_df["aging_rejuv_ratio"] = featured_df["composite_aging_score"] / (
        ...
        for marker1, marker2 in key_ratios:
            if marker1 in df.columns and marker2 in df.columns:
                featured_df[f"{marker1}_{marker2}_ratio"] = df[marker1] / (
                    df[marker2] + 1e-6
                )
    ...
    def create_target_variable(self, df: pd.DataFrame) -> pd.Series:
        ...
        if "aging_senescence_score" in df.columns:
            senescence_component = -df[
                "aging_senescence_score"
        ...
        if target_components:
            ...
            logger.warning(
                "No biological markers found for target creation. Using synthetic target."
            )
    ...
    def train_ensemble_models(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        models = {
            ...
            "elastic_net": ElasticNet(
                alpha=0.1, l1_ratio=0.5, random_state=self.random_state, max_iter=2000
            ),
        }
        ...
        for name, model in models.items():
            ...
            if hasattr(model, "feature_importances_"):
                importance = pd.Series(
                    model.feature_importances_, index=X_scaled_df.columns
                )
...
def main():
    ...
    if not Path(DE_PATH).exists():
        logger.warning(
            f"Data file {DE_PATH} not found. Creating synthetic data for demonstration."
        )
        ...
        # Add aging markers
        for _category, markers in scorer.aging_markers.items():
            for marker in markers:
                # Simulate aging effect
                synthetic_data[marker] = np.random.lognormal(0, 0.5, n_samples)

        # Add rejuvenation markers
        for _category, markers in scorer.rejuvenation_markers.items():
            for marker in markers:
    ...
    print(
        f"Range: {scored_df['rejuvenation_score'].min():.3f} - {scored_df['rejuvenation_score'].max():.3f}"
    )
    print("\nRejuvenation categories:")
    print(scored_df["rejuvenation_category"].value_counts())
    print("\nTop 10 most rejuvenated samples:")
    top_samples = scored_df.nlargest(10, "rejuvenation_score")[
        ["rejuvenation_score", "rejuvenation_category"]
    ]
    print(top_samples)
    print("=" * 60)
