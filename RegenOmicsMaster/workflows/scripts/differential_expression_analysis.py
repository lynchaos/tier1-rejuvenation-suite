...
class DifferentialExpressionAnalyzer:
    ...
    def load_count_files(self, count_files):
        ...
        logger.info(f"Count matrix created: {self.count_matrix.shape[0]} genes, {self.count_matrix.shape[1]} samples")
    ...
    def create_sample_info(self, samples):
        ...
        for sample in samples:
            ...
            if 'young' in sample.lower() or 'ctrl' in sample.lower():
                ...
                condition = np.random.choice(['young', 'aged', 'rejuvenated'], p=[0.4, 0.4, 0.2])
        ...
        logger.info(f"Sample assignments - Young: {sum(np.array(conditions) == 'young')}, "
    ...
    def filter_low_expression_genes(self):
        ...
        expressed_genes = (cpm_matrix > self.min_count_threshold).sum(axis=1) >= min_samples
        
        self.count_matrix_filtered = self.count_matrix[expressed_genes]
        
        logger.info(f"Kept {self.count_matrix_filtered.shape[0]} genes "
    ...
    def normalize_counts(self, method='TMM'):
        ...
        if method == 'TMM':
            ...
            ref_sample = self.count_matrix_filtered.sum(axis=1)  # Sum across samples as reference
            ...
            for sample in self.count_matrix_filtered.columns:
                ...
                if sufficient_counts.sum() > 100:  # Need enough genes for robust estimation
                    m_values = np.log2((sample_counts[sufficient_counts] + 1) / (ref_sample[sufficient_counts] + 1))
                    
                    # Trim extreme values
                    m_trimmed = np.percentile(m_values, [25, 75])
                    m_filtered = m_values[(m_values >= m_trimmed[0]) & (m_values <= m_trimmed[1])]
        ...
        self.normalized_counts = self.count_matrix_filtered.div(effective_lib_sizes, axis=1) * np.mean(effective_lib_sizes)
    ...
    def fit_negative_binomial_glm(self, counts, condition_vector, batch_vector=None):
        ...
        for i, condition in enumerate(unique_conditions[1:], 1):  # Skip first as reference
            design_matrix = np.column_stack([
                design_matrix,
                (condition_vector == condition).astype(int)
            ])
        
        # Add batch effects if provided
        if batch_vector is not None:
            unique_batches = np.unique(batch_vector)
            for i, batch in enumerate(unique_batches[1:], 1):  # Skip first as reference
                design_matrix = np.column_stack([
                    design_matrix,
                    (batch_vector == batch).astype(int)
                ])
    ...
    def perform_differential_expression(self, reference_condition='young'):
        ...
        """
        logger.info(f"Performing differential expression analysis (reference: {reference_condition})...")
        
        conditions = self.sample_info['condition'].values
        batches = self.sample_info['batch'].values
        
        # Get unique conditions for comparisons
        unique_conditions = [c for c in self.sample_info['condition'].unique() if c != reference_condition]
        
        all_results = []
        
        for gene in self.count_matrix_filtered.index:
            gene_counts = self.count_matrix_filtered.loc[gene].values
            ...
            for condition in unique_conditions:
                ...
                if np.sum(ref_samples) >= 2 and np.sum(test_samples) >= 2:
                    ...
                    all_results.append({
                        ...
                        'lfcSE': abs(log_fc) / max(abs(t_stat), 1e-6),  # Approximate standard error
                        'stat': t_stat,
                        'pvalue': p_value,
                        'ref_mean': ref_mean,
                        'test_mean': test_mean
        ...
        de_results_df['significant'] = (de_results_df['padj'] < 0.05) & (abs(de_results_df['log2_fold_change']) > 1)
        ...
        for comparison in de_results_df['comparison'].unique():
            ...
            n_upregulated = ((comp_results['log2_fold_change'] > 1) & comp_results['significant']).sum()
            n_downregulated = ((comp_results['log2_fold_change'] < -1) & comp_results['significant']).sum()
            
            logger.info(f"{comparison}: {n_significant} significant genes "
    ...
    def create_expression_matrix_for_ml(self):
        ...
        ml_matrix = ml_matrix.merge(self.sample_info.set_index('sample_id'), 
    ...
    def generate_visualizations(self, output_dir='results/differential_expression'):
        ...
        for condition in self.sample_info['condition'].unique():
            ...
            plt.scatter(pca_coords[mask, 0], pca_coords[mask, 1], 
                       c=colors.get(condition, 'gray'), label=condition, alpha=0.7, s=60)
        ...
        for comparison in self.de_results['comparison'].unique():
            ...
            # Plot all genes
            plt.scatter(comp_data['log2_fold_change'], -np.log10(comp_data['padj'] + 1e-300),
                       alpha=0.6, s=20, color='lightgray')
            
            # Highlight significant genes
            sig_data = comp_data[comp_data['significant']]
            if len(sig_data) > 0:
                plt.scatter(sig_data['log2_fold_change'], -np.log10(sig_data['padj'] + 1e-300),
                           alpha=0.8, s=30, color='red')
            
            # Add significance thresholds
            plt.axhline(y=-np.log10(0.05), color='blue', linestyle='--', alpha=0.7, label='p=0.05')
    ...
    def generate_html_report(self, output_path='results/differential_expression/de_analysis_report.html'):
        """
        ...
        total_genes = len(self.de_results) // len(self.de_results['comparison'].unique())
        
        comparison_summaries = []
        for comparison in self.de_results['comparison'].unique():
            comp_data = self.de_results[self.de_results['comparison'] == comparison]
            n_significant = comp_data['significant'].sum()
            n_upregulated = ((comp_data['log2_fold_change'] > 1) & comp_data['significant']).sum()
            n_downregulated = ((comp_data['log2_fold_change'] < -1) & comp_data['significant']).sum()
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Differential Expression Analysis for RegenOmics')
    parser.add_argument('--count-files', nargs='+', required=True, 
                       help='List of featureCounts output files')
    parser.add_argument('--output-dir', default='results/differential_expression',
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            samples = ['young_1', 'young_2', 'young_3', 'aged_1', 'aged_2', 'aged_3', 
                      'rejuvenated_1', 'rejuvenated_2', 'rejuvenated_3']
            ...
            print(f"Results saved: differential_expression.csv, expression_matrix_for_ml.csv")