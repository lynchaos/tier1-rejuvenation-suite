...
from tier1_suite.config.settings import GlobalConfig
...
def run_analysis_with_reporting():
    ...
    reporter = ScientificReporter(
        output_dir="reports",
        run_name="aging_biomarkers_demo"
    )
    ...
    reporter.set_metadata(
        ...
        keywords=["aging", "biomarkers", "machine learning", "multi-omics", "reproducibility"],
    ...
    reporter.add_calibration_plot(
        y_true=y,
        y_prob=y_pred_proba,
        title="Random Forest Calibration Plot",
        n_bins=10
    )
