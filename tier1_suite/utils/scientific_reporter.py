...
class ScientificReporter:
    ...
    def __init__(self, 
        ...
        self.run_dir = self.output_dir / "runs" / datetime.now().strftime("%Y-%m-%d") / self.run_name
        self.figures_dir = self.run_dir / "figures"
        self.data_dir = self.run_dir / "data" 
        self.templates_dir = self.run_dir / "templates"
        
        # Create directories
        for dir_path in [self.run_dir, self.figures_dir, self.data_dir, self.templates_dir]:
...
<body>
    ...
    def _format_ci_filter(self, table: MetricsTable, index: int) -> str:
        ...
        if not hasattr(table, 'ci_lower') or not hasattr(table, 'ci_upper'):
            return "N/A"
        if not table.ci_lower or not table.ci_upper or index >= len(table.ci_lower) or index >= len(table.ci_upper):
            return "N/A"
    ...
    def _calculate_confidence_intervals(self,
                                      ...
                                      method: str,
                                      confidence_level: float,
                                      n_bootstrap: int) -> Tuple[List[float], List[float]]:
    ...
    def add_calibration_plot(self,
        ...
        figure = Figure(
            id=figure_id,
            title=title,
            caption=f"Calibration plot showing the relationship between predicted probabilities and actual outcomes. Perfect calibration would follow the diagonal line.",
    ...
    def add_feature_importance_stability(self,
                                       feature_importances: Dict[str, List[float]],
                                       feature_names: List[str],
                                       title: str = "Feature Importance Stability") -> str:
        """
        ...
        for i, (method, importances_list) in enumerate(feature_importances.items()):
            ...
            axes[i].bar(x_pos, mean_imp[top_idx], yerr=std_imp[top_idx], 
                       capsize=5, alpha=0.7)
            axes[i].set_xticks(x_pos)
            axes[i].set_xticklabels([feature_names[idx] for idx in top_idx], 
        ...
        figure = Figure(
            id=figure_id,
            title=title,
            caption=f"Feature importance stability across multiple runs. Error bars show ±1 standard deviation.",
            file_path=str(figure_path.relative_to(self.run_dir)),
    ...
    def add_umap_stability_heatmap(self,
                                 embeddings_list: List[np.ndarray],
                                 labels: Optional[np.ndarray] = None,
                                 title: str = "UMAP Stability Heatmap") -> str:
        """
        ...
        figure = Figure(
            id=figure_id,
            title=title,
            caption=f"UMAP embedding stability heatmap showing mean pairwise distances between runs after Procrustes alignment. Lower values indicate more stable embeddings.",
            file_path=str(figure_path.relative_to(self.run_dir)),
            figure_type='umap'
        )
    ...
    def _procrustes_align(self, X: np.ndarray, Y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def add_custom_figure(self,
                         figure_path: Union[str, Path],
                         title: str,
                         caption: str,
                         figure_type: str = "custom") -> str:
        """
    ...
    def generate_complete_report(self,
                               ...
                               template_name: str = "scientific_report.html") -> Dict[str, Path]:
        """
