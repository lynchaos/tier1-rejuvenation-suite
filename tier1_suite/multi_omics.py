...
class MultiOmicsAnalyzer:
    ...
    def load_omics_data(
        self, data_files: List[str], data_types: Optional[List[str]] = None
        ...
        for i, file_path in enumerate(data_files):
            ...
            if self.verbose:
                print(
                    f"Loaded {data_type}: {data.shape[0]} samples, {data.shape[1]} features"
                )
    ...
    def _fit_autoencoder_integration(
        self, omics_data: Dict[str, pd.DataFrame]
    ) -> Dict[str, Any]:
    ...
    def _save_integration_models(
        self, integration_results: Dict[str, Any], output_path: Path
    ):
    ...
    def generate_embeddings(
        ...
        if not self.is_fitted:
            raise ValueError(
                "Models not fitted. Call fit_integration() or load_models() first."
            )
        ...
        if integration_results["method"] in ["mofa_pca", "pca"]:
            ...
            factors = integration_results["encoder_model"].predict(X_combined)[
                :, : self.n_factors
            ]
        else:
        ...
        if pathway_analysis:
            pathway_scores = self.bio_integrator.analyze_pathway_enrichment(
                factors, omics_data
            )
    ...
    def evaluate_integration(
        ...
        evaluation_results = {"summary": {}, "detailed_metrics": {}}
        ...
        if factor_cols:
            ...
            evaluation_results["summary"]["explained_variance"] = np.var(
                factors, axis=0
            ).sum()
            evaluation_results["summary"]["factor_correlations"] = np.corrcoef(
                factors.T
            ).mean()

        # Biological validation if requested
        if biomarker_validation:
            bio_validation = self.bio_integrator.validate_integration_biology(
                embeddings
            )
            evaluation_results["summary"]["biological_score"] = bio_validation.get(
                "overall_score", 0.0
            )
    ...
    def run_complete_pipeline(
        ...
        if not skip_fitting:
            ...
            integration_results = self.fit_integration(
                data_files=data_files, output_dir=str(output_path / "models")
            )
        ...
        if not skip_evaluation:
            ...
            eval_results = self.evaluate_integration(
                embeddings=embeddings, output_dir=str(output_path / "evaluation")
            )
