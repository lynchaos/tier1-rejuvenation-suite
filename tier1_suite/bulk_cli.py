...
def fit(
    ...
    validation_split: float = typer.Option(0.2, "--val-split", help="Validation split ratio"),
    cv_folds: int = typer.Option(5, "--cv-folds", help="Cross-validation folds"),
    models: Optional[List[str]] = typer.Option(None, "--models", "-m", help="Models to train (rf,xgb,lgb,svm)"),
    biomarker_validation: bool = typer.Option(True, "--biomarker-val/--no-biomarker-val", help="Enable biomarker validation"),
...
def predict(
    ...
    ensemble_method: str = typer.Option("voting", "--ensemble", help="Ensemble method (voting/stacking/mean)"),
    confidence_intervals: bool = typer.Option(True, "--ci/--no-ci", help="Calculate confidence intervals"),
    biomarker_analysis: bool = typer.Option(True, "--biomarker/--no-biomarker", help="Include biomarker analysis"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
...
def validate(
    ...
    ground_truth_file: Optional[str] = typer.Option(None, "--truth", help="Ground truth file"),
    output_dir: str = typer.Option("validation_results", "--output", "-o", help="Output directory"),
    generate_report: bool = typer.Option(True, "--report/--no-report", help="Generate validation report")
):
    """
