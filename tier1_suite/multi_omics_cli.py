...
multi_app = typer.Typer(
    name="multi",
    help="Multi-omics integration and evaluation",
...
def fit(
    ...
    output_dir: str = typer.Argument(..., help="Output directory for integration models"),
    data_types: Optional[List[str]] = typer.Option(None, "--types", help="Data types (rna,protein,metabolite,etc)"),
    integration_method: str = typer.Option("mofa", "--method", help="Integration method (mofa/ae/vae/pca)"),
    n_factors: int = typer.Option(10, "--factors", help="Number of latent factors"),
    batch_correction: bool = typer.Option(True, "--batch/--no-batch", help="Enable batch correction"),
    biomarker_guided: bool = typer.Option(True, "--biomarker/--no-biomarker", help="Use biomarker guidance"),
    ...
    """
    console.print(f"🔬 [bold blue]Starting multi-omics integration fitting...[/bold blue]")
...
def embed(
    data_files: List[str] = typer.Argument(..., help="Input multi-omics data files"),
    model_dir: str = typer.Argument(..., help="Directory with trained integration models"),
    ...
    umap_embed: bool = typer.Option(True, "--umap/--no-umap", help="Compute UMAP embedding"),
    pathway_analysis: bool = typer.Option(True, "--pathway/--no-pathway", help="Include pathway analysis"),
    generate_plots: bool = typer.Option(True, "--plots/--no-plots", help="Generate embedding plots"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
...
def eval(
    ...
    reference_data: Optional[str] = typer.Option(None, "--reference", help="Reference dataset for validation"),
    output_dir: str = typer.Option("evaluation_results", "--output", "-o", help="Output directory"),
    metrics: Optional[List[str]] = typer.Option(["silhouette", "ari", "nmi"], "--metrics", help="Evaluation metrics"),
    biomarker_validation: bool = typer.Option(True, "--biomarker/--no-biomarker", help="Biomarker-based validation"),
    pathway_enrichment: bool = typer.Option(True, "--pathway/--no-pathway", help="Pathway enrichment analysis"),
    generate_report: bool = typer.Option(True, "--report/--no-report", help="Generate evaluation report"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
...
def discover_biomarkers(
    ...
    output_dir: str = typer.Argument(..., help="Output directory for biomarker results"),
    discovery_method: str = typer.Option("integrated_shap", "--method", help="Discovery method (shap/lime/gradient)"),
    top_n: int = typer.Option(100, "--top-n", help="Number of top biomarkers to report"),
    pathway_filter: bool = typer.Option(True, "--pathway-filter/--no-filter", help="Filter by known pathways"),
    validation_split: float = typer.Option(0.3, "--val-split", help="Validation split for biomarker testing"),
    generate_plots: bool = typer.Option(True, "--plots/--no-plots", help="Generate biomarker plots"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
    ...
    try:
        ...
        console.print(f"🎯 Discovered {len(discovery_results['top_biomarkers'])} biomarkers")
        console.print(f"📂 Results saved to: [cyan]{output_dir}[/cyan]")
...
def pipeline(
    ...
    config_file: Optional[str] = typer.Option(None, "--config", help="Configuration file (YAML)"),
    integration_method: str = typer.Option("mofa", "--method", help="Integration method"),
    skip_fitting: bool = typer.Option(False, "--skip-fit", help="Skip model fitting"),
    skip_embedding: bool = typer.Option(False, "--skip-embed", help="Skip embedding generation"),
    skip_evaluation: bool = typer.Option(False, "--skip-eval", help="Skip evaluation"),
    skip_discovery: bool = typer.Option(False, "--skip-discovery", help="Skip biomarker discovery"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
    ...
    """
    console.print(f"🚀 [bold blue]Starting complete multi-omics pipeline...[/bold blue]")
