...
sc_app = typer.Typer(
    name="sc",
    help="Single-cell analysis pipeline",
...
@sc_app.command()
def run_qc(
    input_file: str = typer.Argument(..., help="Input single-cell data file (H5AD/H5/CSV)"),
    ...
    mito_threshold: float = typer.Option(20.0, "--mito-thresh", help="Mitochondrial gene threshold (%)"),
    doublet_detection: bool = typer.Option(True, "--doublets/--no-doublets", help="Enable doublet detection"),
    generate_plots: bool = typer.Option(True, "--plots/--no-plots", help="Generate QC plots"),
...
def run_embed(
    ...
    n_neighbors: int = typer.Option(15, "--neighbors", help="Number of neighbors for UMAP"),
    resolution: float = typer.Option(0.5, "--resolution", help="Clustering resolution"),
    methods: Optional[List[str]] = typer.Option(["umap", "tsne"], "--methods", help="Embedding methods"),
    batch_correct: bool = typer.Option(False, "--batch-correct/--no-batch", help="Enable batch correction"),
    batch_key: Optional[str] = typer.Option(None, "--batch-key", help="Batch correction key"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
    ...
    """
    console.print(f"🎯 [bold blue]Starting dimensionality reduction and embedding...[/bold blue]")
...
def cluster(
    ...
    method: str = typer.Option("leiden", "--method", help="Clustering method (leiden/louvain)"),
    resolution: float = typer.Option(0.5, "--resolution", help="Clustering resolution"),
    key_added: str = typer.Option("clusters", "--key", help="Key for storing clusters"),
    biomarker_annotation: bool = typer.Option(True, "--annotate/--no-annotate", help="Annotate with biomarkers"),
    generate_plots: bool = typer.Option(True, "--plots/--no-plots", help="Generate cluster plots"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
...
def paga(
    ...
    cluster_key: str = typer.Option("clusters", "--cluster-key", help="Clustering key for PAGA"),
    root_cluster: Optional[str] = typer.Option(None, "--root", help="Root cluster for trajectory"),
    compute_pseudotime: bool = typer.Option(True, "--pseudotime/--no-pseudotime", help="Compute pseudotime"),
    rejuvenation_analysis: bool = typer.Option(True, "--rejuv/--no-rejuv", help="Rejuvenation trajectory analysis"),
    generate_plots: bool = typer.Option(True, "--plots/--no-plots", help="Generate PAGA plots"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
...
def pipeline(
    ...
    skip_clustering: bool = typer.Option(False, "--skip-cluster", help="Skip clustering"),
    skip_trajectories: bool = typer.Option(False, "--skip-traj", help="Skip trajectory analysis"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
):
    """
    ...
    """
    console.print(f"🚀 [bold blue]Starting complete single-cell pipeline...[/bold blue]")
