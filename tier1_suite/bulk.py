...
class BulkAnalyzer:
    ...
    def fit_models(
        ...
        if target_column is None:
            ...
            if self.biomarker_validation:
                ...
                raise ValueError("No target column specified and biomarker validation disabled")
        else:
    ...
    def predict(
        ...
        if not self.is_fitted:
            raise ValueError("Models not fitted. Call fit_models() or load_models() first.")
        ...
        if ensemble_method == "voting" or ensemble_method == "mean":
            ...
            ensemble_pred = np.mean(list(predictions.values()), axis=0)  # Default to mean
        ...
        if biomarker_analysis and self.biomarker_validation:
            biomarker_scores = self.bio_validator.calculate_biological_confidence_intervals(data)
    ...
    def validate_predictions(
        ...
        pred_col = 'ensemble_prediction' if 'ensemble_prediction' in predictions.columns else predictions.columns[0]
        ...
        if self.biomarker_validation and 'biomarker_confidence' in predictions.columns:
            bio_corr = np.corrcoef(predictions[pred_col], predictions['biomarker_confidence'])[0, 1]
