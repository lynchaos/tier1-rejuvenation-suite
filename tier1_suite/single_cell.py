...
from SingleCellRejuvenationAtlas.python.biologically_validated_analyzer import BiologicallyValidatedAnalyzer
...
class SingleCellAnalyzer:
    ...
    def run_qc(
        ...
        if doublet_detection:
            try:
                ...
                doublet_scores, predicted_doublets = scrub.scrub_doublets(verbose=self.verbose)
        ...
        qc_results = {
            ...
            'doublets_removed': sum(adata.obs.get('predicted_doublet', [])) if doublet_detection else 0,
    ...
    def cluster_cells(
        ...
        if biomarker_annotation:
            annotations = self.bio_analyzer.annotate_clusters_with_biomarkers(adata, key_added)
    ...
    def run_paga_analysis(
        ...
        if rejuvenation_analysis:
            rejuv_analysis = self.bio_analyzer.analyze_rejuvenation_trajectories(adata, cluster_key)
