...
class BiologicallyValidatedRejuvenationAnalyzer:
    ...
    def __init__(self, adata: ad.AnnData, random_state: int = 42):
        ...
        self.aging_gene_sets = {
            'cellular_senescence': {
                'core_markers': ['CDKN1A', 'CDKN2A', 'TP53', 'RB1'],  # Core senescence (Campisi, 2013)
                'sasp_factors': ['IL1A', 'IL1B', 'IL6', 'IL8', 'TNF', 'CXCL1'],  # SASP (Coppé et al., 2008)
                'senescence_associated': ['GLB1', 'LMNB1', 'HMGB1']  # SA markers (Hernandez-Segura et al., 2018)
            },
            'dna_damage': {
                'repair_genes': ['ATM', 'ATR', 'BRCA1', 'BRCA2', 'RAD51'],  # DNA repair (Jackson & Bartek, 2009)
                'damage_response': ['H2AFX', 'CHEK1', 'CHEK2', 'MDC1', 'PARP1']  # Damage signaling
            },
            'oxidative_stress': {
                'antioxidants': ['SOD1', 'SOD2', 'CAT', 'GPX1', 'PRDX1'],  # Antioxidant defense
                'stress_response': ['NRF2', 'HMOX1', 'NQO1', 'GCLC']  # Stress response (Sykiotis & Bohmann, 2008)
            },
            'mitochondrial_dysfunction': {
                'biogenesis': ['PGC1A', 'NRF1', 'TFAM', 'PPARGC1A'],  # Mitochondrial biogenesis
                'function': ['COX4I1', 'ATP5A1', 'NDUFS1', 'SDHA']  # Mitochondrial function
            }
        }
        ...
        self.rejuvenation_gene_sets = {
            'pluripotency': {
                'yamanaka_factors': ['POU5F1', 'SOX2', 'KLF4', 'MYC'],  # Yamanaka factors (Takahashi & Yamanaka, 2006)
                'pluripotency_network': ['NANOG', 'UTF1', 'DPPA4', 'LIN28A'],  # Extended network (Boyer et al., 2005)
                'reprogramming': ['GDF3', 'LEFTY1', 'LEFTY2', 'NODAL']  # Reprogramming factors
            },
            'longevity': {
                'sirtuins': ['SIRT1', 'SIRT3', 'SIRT6', 'SIRT7'],  # Sirtuin family (Haigis & Sinclair, 2010)
                'foxo_pathway': ['FOXO1', 'FOXO3', 'FOXO4', 'FOXO6'],  # FOXO transcription factors
                'longevity_genes': ['KLOTHO', 'TERT', 'TERC']  # Longevity-associated genes
            },
            'autophagy': {
                'core_autophagy': ['ATG5', 'ATG7', 'ATG12', 'BECN1', 'ULK1'],  # Autophagy machinery
                'selective_autophagy': ['SQSTM1', 'NBR1', 'OPTN'],  # Selective autophagy (Klionsky et al., 2016)
                'mitophagy': ['PINK1', 'PRKN', 'BNIP3', 'FUNDC1']  # Mitochondrial autophagy
            },
            'metabolic_health': {
                'ampk_pathway': ['PRKAA1', 'PRKAA2', 'PRKAB1', 'PRKAG1'],  # AMPK energy sensing
                'metabolic_regulators': ['PPARA', 'PPARG', 'NRF1', 'ESRRA']  # Metabolic transcription factors
            }
        }
        
        # Cell type-specific aging signatures (from literature)
        self.celltype_aging_signatures = {
            'fibroblast': ['COL1A1', 'COL1A2', 'FN1', 'ACTA2'],  # Fibroblast aging (Hernandez-Segura et al., 2017)
            'endothelial': ['PECAM1', 'VWF', 'CDH5', 'NOS3'],    # Endothelial aging (Ungvari et al., 2010)
            'immune': ['CD68', 'CD14', 'CD3E', 'CD19'],          # Immune aging (Fulop et al., 2010)
            'epithelial': ['KRT14', 'KRT5', 'TP63', 'ITGB4'],   # Epithelial aging (Blanpain & Fuchs, 2014)
            'neural': ['MAP2', 'TUBB3', 'SYN1', 'GFAP']         # Neural aging (Mattson & Arumugam, 2018)
        }
    ...
    def preprocess_data_with_biological_validation(self) -> None:
        ...
        print(f"After quality filtering: {self.adata.n_obs} cells, {self.adata.n_vars} genes")
    ...
    def _prioritize_biological_markers(self) -> None:
        ...
        n_bio_genes = sum(1 for gene in biological_genes if gene in self.adata.var_names)
        print(f"Prioritized {n_bio_genes} biological markers in variable gene selection")
    ...
    def calculate_aging_signatures(self) -> None:
        ...
        for category, gene_sets in self.aging_gene_sets.items():
            for signature_name, genes in gene_sets.items():
                ...
                if available_genes:
                    ...
                    print(f"Calculated {signature_key}: {len(available_genes)}/{len(genes)} genes")
                else:
        ...
        aging_scores = [col for col in self.adata.obs.columns if col.startswith('aging_')]
        if aging_scores:
            self.adata.obs['composite_aging_score'] = self.adata.obs[aging_scores].mean(axis=1)
        
        # Calculate composite rejuvenation score  
        rejuv_scores = [col for col in self.adata.obs.columns if col.startswith('rejuv_')]
        if rejuv_scores:
            self.adata.obs['composite_rejuvenation_score'] = self.adata.obs[rejuv_scores].mean(axis=1)
        
        # Calculate aging-rejuvenation balance
        if 'composite_aging_score' in self.adata.obs and 'composite_rejuvenation_score' in self.adata.obs:
        ...
        for res in resolutions:
            ...
            if leiden_key in self.adata.obs:
                ...
                try:
                    ...
                    print(f"Could not calculate silhouette score for resolution {res}: {e}")
                    continue
    ...
    def _calculate_aging_pseudotime(self) -> None:
        ...
        try:
            ...
            if 'composite_rejuvenation_score' in self.adata.obs and 'composite_aging_score' in self.adata.obs:
                ...
                potential_roots = np.where(root_score > np.percentile(root_score, 95))[0]
    ...
    def _alternative_aging_analysis(self) -> None:
        ...
        if 'composite_aging_score' in self.adata.obs:
            ...
            print(f"PC{best_pc+1} most correlated with aging (r={max(correlations):.3f})")
    ...
    def _create_reprogramming_target(self) -> np.ndarray:
        ...
        if 'aging_cellular_senescence_core_markers' in self.adata.obs:
            senescence = zscore(self.adata.obs['aging_cellular_senescence_core_markers'])
        ...
        if target_components:
            ...
            target = np.random.beta(2, 3, self.adata.n_obs)  # Skewed toward lower values
    ...
    def generate_biological_plots(self, save_dir: str = "figures/") -> None:
        ...
        if 'leiden' in self.adata.obs:
            sc.pl.umap(
                self.adata, 
                color=['leiden', 'composite_aging_score', 'composite_rejuvenation_score'], 
        ...
        aging_sigs = [col for col in self.adata.obs.columns if col.startswith('aging_')][:4]
        if aging_sigs:
            sc.pl.umap(
                self.adata, 
                color=aging_sigs, 
                save='_aging_signatures.pdf',
                ncols=2
            )
        
        # Rejuvenation signature plots
        rejuv_sigs = [col for col in self.adata.obs.columns if col.startswith('rejuv_')][:4]
        if rejuv_sigs:
            sc.pl.umap(
                self.adata, 
                color=rejuv_sigs, 
