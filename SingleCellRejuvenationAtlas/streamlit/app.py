...
analysis_type = st.sidebar.selectbox(
    ...
    ["Data Overview", "Trajectory Analysis", "Reprogramming Predictor", 
     "Senescence Analysis", "Pluripotency Scoring", "Cell Communication", "Digital Twin Simulator"]
)
...
def load_sample_data():
    ...
    cell_types = np.random.choice(['Young', 'Aged', 'Rejuvenated'], size=n_cells, p=[0.4, 0.4, 0.2])
    ...
    gene_metadata = pd.DataFrame({
        ...
        'gene_type': np.random.choice(['protein_coding', 'lncRNA', 'miRNA'], size=n_genes, p=[0.8, 0.15, 0.05])
    })
...
if analysis_type == "Data Overview":
    ...
    fig_dist = px.histogram(cell_metadata, x='cell_type', title="Cell Type Distribution",
                           color='cell_type', color_discrete_sequence=px.colors.qualitative.Set3)
    st.plotly_chart(fig_dist, use_container_width=True)
    
    # UMAP plot
    fig_umap = px.scatter(cell_metadata, x='UMAP_1', y='UMAP_2', color='cell_type',
                         title="UMAP: Cell Type Distribution", hover_data=['cell_id'])
    ...
    fig_traj = px.scatter(cell_metadata, x='UMAP_1', y='UMAP_2', 
                         color='senescence_score', size='pluripotency_score',
    ...
    pseudotime = np.argsort(cell_metadata['senescence_score'] - cell_metadata['pluripotency_score'])
    cell_metadata['pseudotime'] = pseudotime
    
    fig_pseudo = px.scatter(cell_metadata, x='pseudotime', y='senescence_score',
                           color='cell_type', title="Pseudotime vs Senescence Score")
    ...
    fig_reprog = px.scatter(cell_metadata, x='senescence_score', y='pluripotency_score',
                           color='reprogramming_potential', size='reprogramming_potential',
    ...
    top_candidates = cell_metadata.nlargest(10, 'reprogramming_potential')[['cell_id', 'cell_type', 'reprogramming_potential']]
    ...
    fig_sen_dist = px.histogram(cell_metadata, x='senescence_score', color='cell_type',
    ...
    fig_sen_box = px.box(cell_metadata, x='cell_type', y='senescence_score',
    ...
    fig_pluri_dist = px.histogram(cell_metadata, x='pluripotency_score', color='cell_type',
    ...
    fig_high_pluri = px.scatter(high_pluripotency, x='UMAP_1', y='UMAP_2',
                               color='pluripotency_score', size='pluripotency_score',
    ...
    communication_data = pd.DataFrame({
        ...
        'target_cell_type': ['Aged', 'Rejuvenated', 'Young', 'Rejuvenated', 'Young'],
    ...
    with col1:
        ...
        initial_pluripotency = st.slider("Initial Pluripotency Level", 0.0, 1.0, 0.2, 0.05)
    
    with col2:
        autophagy_rate = st.slider("Autophagy Enhancement", 0.0, 2.0, 1.0, 0.1)
        inflammation_suppression = st.slider("Inflammation Suppression", 0.0, 2.0, 1.0, 0.1)
    ...
    def simulate_cellular_dynamics(params):
        ...
        def rejuvenation_ode_system(state, t, params):
            ...
            """
            S, P, A, I, D, M = state  # Senescence, Pluripotency, Autophagy, Inflammation, DNA damage, Metabolism
            ...
            dS_dt = -0.1 * treatment * S * (1 - S) - 0.05 * A * S + 0.03 * I * (1 - S) + 0.02 * D * (1 - S)
            
            # Pluripotency dynamics (increases with treatment, inhibited by senescence)
            dP_dt = 0.08 * treatment * (1 - P) * (1 - 0.5 * S) + 0.04 * A * (1 - P) - 0.02 * P * I
            
            # Autophagy dynamics (enhanced by treatment)
            dA_dt = 0.12 * autophagy_boost * treatment * (1 - A) - 0.08 * A + 0.03 * P * (1 - A)
            
            # Inflammation dynamics (suppressed by treatment)
            dI_dt = -0.15 * inflammation_supp * treatment * I + 0.05 * S * (1 - I) - 0.06 * A * I
            ...
            dM_dt = 0.1 * metabolic_opt * treatment * (1 - M) + 0.06 * A * (1 - M) - 0.03 * S * M - 0.02 * I * M
        ...
        # Solve ODE system
        solution = odeint(rejuvenation_ode_system, initial_conditions, time_points, args=(params,))
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=['Cellular Aging Markers', 'Rejuvenation Markers', 
                       'Cellular Processes', 'Composite Rejuvenation Score',
                       'Treatment Response', 'Cellular State Phase Plot'],
    ...
    fig.add_trace(go.Scatter(x=time_points, y=senescence_traj, name='Senescence', 
                            line=dict(color='red', width=3)), row=1, col=1)
    fig.add_trace(go.Scatter(x=time_points, y=inflammation_traj, name='Inflammation',
                            line=dict(color='orange', width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=time_points, y=dna_damage_traj, name='DNA Damage',
                            line=dict(color='darkred', width=2)), row=1, col=1)
    
    # Plot 2: Rejuvenation markers
    fig.add_trace(go.Scatter(x=time_points, y=pluripotency_traj, name='Pluripotency',
                            line=dict(color='green', width=3)), row=1, col=2)
    fig.add_trace(go.Scatter(x=time_points, y=autophagy_traj, name='Autophagy',
                            line=dict(color='blue', width=2)), row=1, col=2)
    fig.add_trace(go.Scatter(x=time_points, y=metabolic_health_traj, name='Metabolism',
                            line=dict(color='cyan', width=2)), row=1, col=2)
    
    # Plot 3: Cellular processes dynamics
    process_efficiency = autophagy_traj * (1 - inflammation_traj) * (1 - dna_damage_traj)
    fig.add_trace(go.Scatter(x=time_points, y=process_efficiency, name='Process Efficiency',
                            line=dict(color='purple', width=3)), row=2, col=1)
    
    # Plot 4: Composite rejuvenation score
    fig.add_trace(go.Scatter(x=time_points, y=rejuvenation_score, name='Rejuvenation Score',
                            line=dict(color='darkgreen', width=4)), row=2, col=2)
    
    # Plot 5: Treatment response curve
    treatment_response = np.exp(-0.1 * time_points) * (1 - treatment_strength) + treatment_strength
    fig.add_trace(go.Scatter(x=time_points, y=treatment_response, name='Treatment Effect',
                            line=dict(color='gold', width=3)), row=3, col=1)
    
    # Plot 6: Phase plot (Senescence vs Pluripotency)
    fig.add_trace(go.Scatter(x=senescence_traj, y=pluripotency_traj, 
                            mode='lines+markers', name='Cellular Trajectory',
                            line=dict(color='magenta', width=2),
                            marker=dict(size=3)), row=3, col=2)
    
    fig.update_layout(height=900, title_text="Digital Twin: Comprehensive Cellular Dynamics")
...
# Footer
st.markdown("---")
st.markdown("Single-Cell Rejuvenation Atlas | Interactive Cellular Aging Analysis Platform")